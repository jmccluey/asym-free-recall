function report_file = beh_report_asymFR(data, res_dir, report_name, ...
                                         compile)
%BEH_REPORT_ASYMFR  Behavioral report for asymFR.
%
%  report_file = beh_report_asymFR(data, res_dir, report_name, compile)
%
%  INPUTS:
%          data:  asymFR data structure
%       res_dir:  directory in which to place the resulting report,
%                 e.g., '~/reports/asymfr'
%   report_name:  file name for the LaTeX report ('asymfr_beh_report')
...
%%% SPC %%%
% general, less semantically related
% make grand average and subject plots
f = @plot_spc;
f_in = {};
[ga_file, subj_files] = plot_ga_subj(res.gen.spc.subj, subjects, f, f_in, fig_dir, 'spc_gen_%s.eps', PRINT);

% add to our list of files
fig_files = add_fig_files(fig_files, 'SPCgen', {ga_file, subj_files{:}});

% specific, more semantically related
% make grand average and subject plots
f = @plot_spc;
f_in = {};
[ga_file, subj_files] = plot_ga_subj(res.spec.spc.subj, subjects, f, f_in, fig_dir, 'spc_spec_%s.eps', PRINT);

% add to our list of files
fig_files = add_fig_files(fig_files, 'SPCspec', {ga_file, subj_files{:}});
...
f = @plot_crp;
f_in = {};
[ga_file, subj_files] = plot_ga_subj(res.gen.crp.subj, subjects, f, f_in, fig_dir, 'crp_gen_%s.eps', PRINT);

% add to our list of files
fig_files = add_fig_files(fig_files, 'CRPgen', {ga_file, subj_files{:}});

% specific, more semantically related
% make grand average and subject plots
f = @plot_crp;
f_in = {};
[ga_file, subj_files] = plot_ga_subj(res.spec.crp.subj, subjects, f, f_in, fig_dir, 'crp_spec_%s.eps', PRINT);

% add to our list of files
fig_files = add_fig_files(fig_files, 'CRPspec', {ga_file, subj_files{:}});
...
f = @plot_spc;
f_in = {};
[ga_file, subj_files] = plot_ga_subj(res.gen.pfr.subj, subjects, f, f_in, fig_dir, 'pfr_gen_%s.eps', PRINT);

% add to our list of files
fig_files = add_fig_files(fig_files, 'PFRgen', {ga_file, subj_files{:}});

% specific, more semantically related
% make grand average and subject plots
f = @plot_spc;
f_in = {};
[ga_file, subj_files] = plot_ga_subj(res.spec.pfr.subj, subjects, f, f_in, fig_dir, 'pfr_spec_%s.eps', PRINT);

% add to our list of files
fig_files = add_fig_files(fig_files, 'PFRspec', {ga_file, subj_files{:}});
...
landscape = true;
longtable(table, header, report_ltx, 'AsymFR Behavioral Analysis', landscape);
