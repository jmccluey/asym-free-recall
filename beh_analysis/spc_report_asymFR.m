function report_file = spc_report_asymFR(data, res_dir, report_name, ...
                                         compile)
%SPC_REPORT_ASYMFR  Behavioral SPC report for asymFR, across categories.
%
%  report_file = spc_report_asymFR(data, res_dir, report_name, compile)
%
%  INPUTS:
%          data:  asymFR data structure
%       res_dir:  directory in which to place the resulting report,
%                 e.g., '~/reports/asymfr'
%   report_name:  file name for the LaTeX report ('asymfr_spc_report')
...
f = @plot_spc;
f_in = {};
[ga_file, subj_files] = plot_ga_subj(res.anim.spc.subj, subjects, f, f_in, fig_dir, 'spc_anim_%s.eps', PRINT);
...
f = @plot_spc;
f_in = {};
[ga_file, subj_files] = plot_ga_subj(res.food.spc.subj, subjects, f, f_in, fig_dir, 'spc_food_%s.eps', PRINT);
...
f = @plot_spc;
f_in = {};
[ga_file, subj_files] = plot_ga_subj(res.tool.spc.subj, subjects, f, f_in, fig_dir, 'spc_tool_%s.eps', PRINT);
...
f = @plot_spc;
f_in = {};
[ga_file, subj_files] = plot_ga_subj(res.vehi.spc.subj, subjects, f, f_in, fig_dir, 'spc_vehi_%s.eps', PRINT);
...
landscape = false;
longtable(table, header, report_ltx, 'AsymFR SPC Report', landscape);
