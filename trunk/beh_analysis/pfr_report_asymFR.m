function report_file = pfr_report_asymFR(data, res_dir, report_name, ...
                                         compile)
%PFR_REPORT_ASYMFR  Behavioral PFR report for asymFR, across categories.
%
%  report_file = pfr_report_asymFR(data, res_dir, report_name, compile)
%
%  INPUTS:
%          data:  asymFR data structure
%       res_dir:  directory in which to place the resulting report,
%                 e.g., '~/reports/asymfr'
%   report_name:  file name for the LaTeX report ('asymfr_pfr_report')
...
f = @plot_spc;
f_in = {};
[ga_file, subj_files] = plot_ga_subj(res.anim.pfr.subj, subjects, f, f_in, fig_dir, 'pfr_anim_%s.eps', PRINT);
...
f = @plot_spc;
f_in = {};
[ga_file, subj_files] = plot_ga_subj(res.food.pfr.subj, subjects, f, f_in, fig_dir, 'pfr_food_%s.eps', PRINT);
...
f = @plot_spc;
f_in = {};
[ga_file, subj_files] = plot_ga_subj(res.tool.pfr.subj, subjects, f, f_in, fig_dir, 'pfr_tool_%s.eps', PRINT);
...
f = @plot_spc;
f_in = {};
[ga_file, subj_files] = plot_ga_subj(res.vehi.pfr.subj, subjects, f, f_in, fig_dir, 'pfr_vehi_%s.eps', PRINT);
...
landscape = false;
longtable(table, header, report_ltx, 'AsymFR PFR Report', landscape);
